...
def get_dates_to_fetch() -> list:
    """
    Ưu tiên: PREDICT_DATE env var (chạy thủ công chọn ngày)
    Thứ 6 tự động → lấy 4 ngày (T6+T7+CN+T2)
    Các ngày khác → chỉ hôm nay
    """
    # Nếu có truyền ngày cụ thể qua input
    predict_date = os.environ.get("PREDICT_DATE", "").strip()
    if predict_date:
        print(f"📌 Chạy thủ công cho ngày: {predict_date}")
        return [predict_date]
